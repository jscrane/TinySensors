...
my $KEY = "5a33baa5f85d1e436a14a25815655f76";
my $WS = "Rathgar,IE";
...
$INTERVAL = defined($opt{i}) ? $opt{i} : $INTERVAL;
...
while (1) {
	...
	my $content = get("http://api.openweathermap.org//data/2.5/weather?q=$WS&units=metric&appid=$KEY&mode=xml");
	my $w = $xml->XMLin($content);
